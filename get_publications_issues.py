...
BASE_URLS = { "abs" : "https://www.abs.gov.au", "rba":"https://www.rba.gov.au/statistics/"}
BASE_PATH = Path(__file__).parent

Release = namedtuple('Release', ['agency', 'name', 'address','release_date','type', 'file'])


publications = {
    "house_prices": ("abs", "/statistics/economy/price-indexes-and-inflation/residential-property-price-indexes-eight-capital-cities", "641606.xlsx"),
    "weekly_earnings": ("abs", "/statistics/labour/earnings-and-working-conditions/average-weekly-earnings-australia", "6302002.xlsx"),
    "house_finance": ("abs", "/statistics/economy/finance/lending-indicators", "560103.xlsx"),
    "inflation": ("abs",  "/statistics/economy/price-indexes-and-inflation/consumer-price-index-australia", "640101.xlsx"),
    "unemployment": ("abs", "/statistics/labour/employment-and-unemployment/labour-force-australia", "6202001.xlsx"),
...
def get_latest_release(html:str, pub:tuple)->list:
    ...
    div = soup.find(id="block-views-block-topic-releases-listing-topic-latest-release-block")
    if div:
        ...
        if latest_release:
            ...
            try:
                ...
                release_date = None
            latests.append(Release(name=latest_release.text, 
                                   address=address_url, 
                                   type="latest", 
...
def get_future_releases(html:str, pub:tuple)->list:
    ...
    future_releases = []
    soup = BeautifulSoup(html, 'html.parser')
    div = soup.find(id="block-views-block-topic-releases-listing-future-releases-block")
    ...
    if div:
        ...
        for views_row in views_row_divs:
            ...
            future_releases.append(Release(name=release_title, 
...
def get_rba_relase(stat: str, pub:tuple)->None:
    ...
    release_date = dt.strftime('%Y-%m-%d')
    latests = []
    latests.append(Release(name=stat, 
                            address=address, 
                            type="latest", 
                            release_date=release_date,
                            agency=agency,
...
release_df.to_parquet(BASE_PATH / "database" / "releases.parquet", index=False, engine="pyarrow")
