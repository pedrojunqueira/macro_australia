...
def merge_columns(
    ...
    df[second_col] = pd.concat(
        [
            df[first_col].iloc[: last_col1_index + 1],
            df[second_col].iloc[last_col1_index + 1 :],
        ]
    )
...
def pbi_file(data_files_path: Path):
    data = data_files_path / 'database' / 'data.parquet'
    ...
    filter = df_data['variable'].isin(target_series)
    ...
    for s, head in zip(target_series, headers):
        ...
        serie_df['month'] = (
            serie_df['month'].dt.to_period('M').dt.to_timestamp()
        )
    ...
    df['total_new_loans_no'] = (
        df['contruction_no'] + df['new_built_no'] + df['existing_dwelling_no']
    )
    df['total_new_loans_value'] = (
        df['contruction_value']
        + df['new_built_value']
        + df['existing_dwelling_value']
    )
    df['average_loan_value'] = (
        df['total_new_loans_value'] / df['total_new_loans_no']
    ) * 1000
    df['average_yearly_interest'] = (
        (df['variable_loan_owner'] / 100) * df['average_loan_value']
    ) * 1000
    df['loan_repayment_perc_income'] = df['average_yearly_interest'] / (
        df['weekly_earnings'] * 52
    )
    ...
    df_final.to_csv(data_files_path / 'final.csv', index=False)
